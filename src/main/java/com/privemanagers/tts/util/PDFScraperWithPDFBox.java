...
public class PDFScraperWithPDFBox {
	 ...
	 * File("/home/mkondvilkar/prive-jira/Citi-New/scrapper/kfs_pdfs/new_pdfs");
	 * 
	 * if (folder.isDirectory()) { List<File> files =
	 * Arrays.asList(folder.listFiles());
	 * 
	 * for (File file : files) { if (file.isFile()) {
	 * 
	 * processPdf(file,
	 * "/home/mkondvilkar/prive-jira/Citi-New/scrapper/kfs_pdfs/out_1/" +
	 * file.getName() + ".txt"); // extractFundName(file); } } }
	 * 
	 * 
	 * processPdf1( new File(
	 * "/home/mkondvilkar/prive-jira/Citi-New/scrapper/kfs_pdfs/FS_HK_en_LU1226287529_YES_2017-05-01.pdf"
	 * ), "/home/mkondvilkar/prive-jira/Citi-New/scrapper/kfs_pdfs/out/out.txt"
	 * );
	 * 
	 * 
	 * // extractFundName(new // File(
	 * "/home/mkondvilkar/prive-jira/Citi-New/scrapper/kfs_pdfs/FS_HK_en_LU1322973634_YES_2017-09-25.pdf"
	 * ));
	 * 
	 * }
	 */
	...
	private static String processPdfEnglish(File file, String... ignoreLines) throws Exception {
		...
		PDDocument doc = null;
		try {
			System.out.println("Inside processPdfEnglish");
			// load pdf
			...
			while ((line = reader.readLine()) != null) {
				...
				if (ignoreLines[0] != null && ignoreLines[0].toLowerCase().replaceAll("[^A-Za-z0-9\\. ]", "")
						.contains(line.toLowerCase().replaceAll("[^A-Za-z0-9\\. ]", ""))) {
					System.out.println("Exact Fund name found in script...ignoring");
					continue;
				}
